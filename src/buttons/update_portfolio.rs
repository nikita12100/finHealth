...
pub async fn handler_update_balance_btn(
    bot: Bot,
    dialogue: MyDialogue,
    q: CallbackQuery,
) -> HandlerResult {
    ...
    match q.data.clone().unwrap().as_str() {
        UpdatePortfolioButton::EDIT_BALANCES => {
            bot.edit_message_text(
                ...
                "Как вы хотите изменить портфель?",
            )
            .await?;

            dialogue
                .update(State::ListenEditPortfolioButtonsCallback)
                .await?;
            bot.send_message(chat_id, "Выберите что изменить:")
                .reply_markup(make_keyboard(1, EditPortfolioButton::VALUES.to_vec()))
                .await?;
        }
        _ => {
            if let Some(portfolio) = Portfolio::get(q.chat_id().unwrap().0) {
                ...
                bot.edit_message_text(
                    chat_id,
                    q.message.clone().unwrap().id(),
                    format!("Редактирование счета {}", chosen_balance),
                )
                .await?;
                bot.send_message(chat_id, "Выберете действие:")
                    .reply_markup(make_keyboard(1, EditAccountButton::VALUES.to_vec()))
                    .await
                    .unwrap();

                dialogue
                    .update(State::GotListenAccountNameListenAccountButtonsCallback(
                        chosen_balance,
                    ))
                    .await?;
            } else {
                bot.send_message(
                    chat_id,
                    "У вас еще нет баланса, введите имя для нового баланса:",
                )
                .await?;
