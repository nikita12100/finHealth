...
impl MockData {
    pub fn create() -> Portfolio {
        ...
        let a3 = Account::new_date("TPAY".parse().unwrap(), 403000, Currency::RUB, AssetLocation::Broker1, AssetType::Bond, Utc.with_ymd_and_hms(2024, 11, 1, 22, 1, 55).unwrap());
        ...
        p.get_account_mut("крипта").unwrap().add_balance_income_date(50000, Utc.with_ymd_and_hms(2024, 11, 2, 21, 1, 55).unwrap());
        p.get_account_mut("TMON").unwrap().add_balance_income_date(5000, Utc.with_ymd_and_hms(2024, 11, 2, 21, 1, 55).unwrap());
        p.get_account_mut("TPAY").unwrap().add_balance_income_date(500, Utc.with_ymd_and_hms(2024, 11, 2, 21, 1, 55).unwrap());
        p.get_account_mut("металл").unwrap().add_balance_income_date(15000, Utc.with_ymd_and_hms(2024, 11, 2, 21, 1, 55).unwrap());
        p.get_account_mut("вклад").unwrap().add_balance_outcome_date(15000, Category::OTHER.to_string(), Utc.with_ymd_and_hms(2024, 11, 2, 21, 1, 55).unwrap());
