...
pub async fn handler_update_account_btn(
    bot: Bot,
    dialogue: MyDialogue,
    account_name: String,
    q: CallbackQuery,
    ...
    match q.data.clone().unwrap().as_str() {
        EditAccountButton::INCOME_AMOUNT => {
            ...
            dialogue
                .update(State::ListenAccountIncomeFor(account_name))
                .await?;
        }
        EditAccountButton::OUTCOME_AMOUNT => {
            ...
            dialogue
                .update(State::ListenAccountOutcomeFor(account_name))
                .await?;
        }
        EditAccountButton::SET_CURRENCY => {
            let current_currency = portfolio
            ...
            bot.edit_message_text(
                chat_id,
                q.message.clone().unwrap().id(),
                format!("Текущая валюта счета {}", current_currency.to_string()),
            )
            .await?;

            dialogue
                .update(State::ListenCurrencyForAccountCallback(account_name))
                .await?;
            bot.send_message(chat_id, "Выберите валюту")
                .reply_markup(make_keyboard_string(1, ButtonCurrency::get_currencies()))
                .await?;
        }
        EditAccountButton::SET_LOCATION => {
            let current_location = portfolio
                .get_account(&*account_name)
                .unwrap()
                .get_location()
                .clone();
            bot.edit_message_text(
                chat_id,
                q.message.clone().unwrap().id(),
                format!("Текущая локация счета {}", current_location.to_string()),
            )
            .await?;

            dialogue
                .update(State::ListenLocationForCallback(account_name))
                .await?;
            bot.send_message(chat_id, "Выберите локацию")
                .reply_markup(make_keyboard_string(1, ButtonLocation::get_locations()))
                .await?;
        }
        EditAccountButton::SET_TYPE => {
            let current_type = portfolio
                .get_account(&*account_name)
                .unwrap()
                .get_type()
                .clone();
            ...
            bot.edit_message_text(
                chat_id,
                q.message.clone().unwrap().id(),
                format!("Текущий тип счета {}", current_type.to_string()),
            )
            .await?;

            dialogue
                .update(State::ListenTypeForCallback(account_name))
                .await?;
            bot.send_message(chat_id, "Выберите тип")
                .reply_markup(make_keyboard_string(1, ButtonType::get_types()))
                .await?;
        }
        EditAccountButton::REMOVE_BALANCE => {
            ...
            bot.send_message(
                chat_id,
                format!("Баланс {} успешно удален", account.get_name()),
            )
            .await?;
        ...
        _ => {
            invalid_input_for_callback(
                ...
                format!(
                    ...
                    EditAccountButton::VALUES.to_vec()
                ),
            )
            .await?;
        }
