...
impl PieChart {
    ...
    pub fn create(
        ...
        log::info!("creating pie chart {}", title_text);
        assert!(parts.len() > 0);
    ...
    fn create_bytes(
        ...
        let mut bytes = vec![0; Self::WIDTH as usize * Self::HEIGHT as usize * 3];
        {
            ...
            let title_style =
                TextStyle::from(("monospace", 50, "italic").into_font()).color(&(BLACK));
            ...
            if let Some(text) = center_text {
                let center_text_style =
                    TextStyle::from(("monospace", 40, "bold").into_font()).color(&(BLACK));
                let text_pos = (center.0 - ((text.len() * 9) as i32), center.1 - 10);
                root.draw_text(&text, &center_text_style.into_text_style(&root), text_pos)
                    .unwrap();
